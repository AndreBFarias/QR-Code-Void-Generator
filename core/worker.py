...
class QRWorker(QThread):
    finished = pyqtSignal(object, int) # Image, Request ID
    error = pyqtSignal(str, int) # Error message, Request ID
    ...
    def __init__(self, request_id, data, ec, size, rounded, logo_img, logo_size, logo_opacity, logo_pos="center", easter_egg=False, fill_color="black", back_color="white"):
        ...
        self.logo_img = logo_img # PIL Image or Path (but prefer Image for SVGs)
    ...
    def run(self):
        try:
            ...
            if self.easter_egg:
                ...
                print(f"DEBUG: Gerando QR com Logo? {bool(self.logo_img)}")
                # print(f"DEBUG: Caminho da Logo: {self.logo_path}") # Removed as it might be an object now
                print(f"DEBUG: Cores Finais -> Back: {self.back_color}, Fill: {self.fill_color}")

                qr_img = self.generator.generate_qr(
                    data=self.data,
                    error_correction=self.ec,
                    box_size=10, # Base size, will be resized
                    border=4,
                    fill_color=self.fill_color,
                    back_color=self.back_color,
                    rounded_modules=self.rounded
                )
                ...
                if self.logo_img:
                    qr_img = add_logo(
                        ...
                        self.logo_size, 
                        self.logo_opacity,
                        position=self.logo_pos,
                        border_width=40 # 4 modules * 10px box_size
                    )
