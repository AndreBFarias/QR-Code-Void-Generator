import os
from PyQt6.QtWidgets import (QWidget, QLabel, QVBoxLayout, QDialog, QGridLayout, 
                             QPushButton, QMenu, QRadioButton, QButtonGroup, QHBoxLayout,
                             QFrame)
...
class SocialPlatformSelector(QWidget):
    platformSelected = pyqtSignal(str, str) # Name, Icon Path
...
class LogoPositionSelector(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        for name, r, c in positions:
            btn = QPushButton()
            btn.setFixedSize(25, 25) # Compact
            btn.setCheckable(True)
...
class PreviewLabel(QLabel):
    ...
    actionRequested = pyqtSignal(str) # "open", "save", "clear", "print"
    ...
    def update_display(self):
        ...
        if self.is_zoomed:
            target_pixmap = self.original_pixmap.scaled(
                int(self.original_pixmap.width() * 1.05),
                int(self.original_pixmap.height() * 1.05),
